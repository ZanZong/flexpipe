...
class WeightedBatchSampler(torch.utils.data.sampler.BatchSampler):
    def __init__(self,
        ...
        for name, new_cfg in self.task_cfg.items():
            if new_cfg.DATASETS.DATASET_NAME in [
                ...
                print(f"local rank={comm.get_local_rank()}, task={name}, sampler=TrainingSampler")
            elif new_cfg.DATASETS.DATASET_NAME in ["BooksWiki"]:
                # block cache
                sampler = NaiveSampler(self.dataset.datasets[name])
                print(f"local rank={comm.get_local_rank()}, task={name}, sampler=NaiveSampler")
                ...
                print(f"local rank={comm.get_local_rank()}, task={name}, sampler=NodeDistributedSampler")
            else:
    ...
    def __iter__(self):
        """ This iterator will gen separate task for each dp rank!! """
        batch = []
        # while True:
        ...
        for i in range(self.task_batch_size[task]):
            try:
                ...
                print("except occur when iterate sampler!!")
                self.unit_sampler_iter[task] = iter(self.unit_sampler[task])
                batch.append(
                    next(self.unit_sampler_iter[task]) + sample_index_offset)
        # print(f"local rank={comm.get_local_rank()}, for task={task}, gen batch size={self.task_batch_size[task]}")
