...
class UnifiedTrainer(TrainerBase):
        ...
        with EventStorage(start_iter) as self.storage:
            try:
                ...
                for self.iter in range(start_iter, max_iter):
                    self.before_step()
                    timers("iter_time").start()
                    self.run_step_torch()
                    timers("iter_time").stop()
                    timers.log_all()
                    self.after_step()
    ...
    def run_min_batch(self):
        ...
        print(f"Rank {comm.get_local_rank()} run_min_batch, taskname={task}, " + \
                f"len input_sample={len(data['input_sample_list'])}, "+ \
                f"len target_sample_list={len(data['target_sample_list'])}, " + \
                    f"len shared_target_set={len(data['shared_target_sets'])}")
