...
class MultiTaskTransformerEncoder(BaseEncoderDecoder):
    ...
    def from_config(cls, cfg):
        ...
        # Register to verify each forward will trigger once backward
        # def forward_post_hook(module, in_, out_):
        #     print("\n Fused encoder (UnifiedBertEncoder) do a forward\n")
        
        # def backward_post_hook(module, in_, out_):
        #     print("\n Fused encoder (UnifiedBertEncoder) do a backward\n")
        
        # ret["fused_encoder"].register_forward_hook(hook=forward_post_hook)
        # ret["fused_encoder"].register_full_backward_hook(hook=backward_post_hook)
    ...
    def _forward(self, batched_inputs):
        ...
        # timers("target_sample_forward").stop()
        
        # timers("shared_target_forward").start()
        ...
        # timers("shared_target_forward").stop()

        timers("loss_process").start()
        loss_inputs = self.loss_prepare(**batched_inputs)
        self.fc_prompt_process(loss_inputs)
        timers("loss_process").stop()
    ...
    def _forward_data(self, data_list:list, task_info:dict, history_states=None, return_all=False):
        ...
        timers("fused_preprocess").stop()
    
        timers("fused_encoders").start()
        ...
        timers("fused_encoders").stop()
        
        timers("fused_postprocess").start()
        postprocess(fused_data_dict, task_info=task_info)
        timers("fused_postprocess").stop()
